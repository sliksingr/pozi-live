...
/*
  BUILDr usage enforcement
  Edit these numbers only when changing daily limits.
  This is backend-only enforcement. Do not put Supabase anon keys here.
*/
const BUILDR_USAGE_LIMITS = {
  guest: 1,
  free: 3,
  consumer: 25,
  pro: 100
...
const BUILDR_USAGE_TABLE = "usage_tracking";

function normalizeAccountType(value, hasUser) {
  const type = String(value || "").toLowerCase().trim();

  if (type === "pro") return "pro";
  if (type === "consumer" || type === "consumer_paid" || type === "paid") return "consumer";
  if (type === "free" || type === "free_account") return "free";
...
function getUsageIdentity({ session_id, user_id, event }) {
  const user = safeUsageId(user_id);
  if (user && user !== "guest" && user !== "anonymous") {
    return "user:" + user;
  }

  const session = safeUsageId(session_id);
  if (session && session !== "guest" && session !== "anonymous") {
  ...
  return "guest:ip:" + safeUsageId(String(ip).split(",")[0]);
}

async function supabaseUsageRest(path, options = {}) {
  const supabaseUrl = process.env.SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error("Usage enforcement missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.");
  }
  ...
  if (!response.ok) {
    throw new Error(
      typeof data === "string"
        ? data
        : data?.message || data?.error || "Supabase usage request failed."
    );
  }
...
async function getUsageRow(identity, usageDate) {
  const rows = await supabaseUsageRest(
    `${BUILDR_USAGE_TABLE}?select=*&session_id=eq.${encodeURIComponent(identity)}&usage_date=eq.${encodeURIComponent(usageDate)}&limit=1`,
    { method: "GET" }
  );
...
async function createUsageRow(identity, usageDate, accountType) {
  const rows = await supabaseUsageRest(BUILDR_USAGE_TABLE, {
    method: "POST",
    headers: {
...
async function updateUsageRow(rowId, nextCount, accountType) {
  const rows = await supabaseUsageRest(
    `${BUILDR_USAGE_TABLE}?id=eq.${encodeURIComponent(rowId)}`,
    {
      method: "PATCH",
      headers: {
        "Prefer": "return=representation"
...
async function checkAndIncrementBuildrUsage({ session_id, user_id, account_type, event }) {
  const hasUser = !!(user_id && user_id !== "guest" && user_id !== "anonymous");
  const accountType = normalizeAccountType(account_type, hasUser);
  const limit = BUILDR_USAGE_LIMITS[accountType] ?? BUILDR_USAGE_LIMITS.guest;
  const usageDate = todayKeyUTC();
  const identity = getUsageIdentity({ session_id, user_id, event });

  let row = await getUsageRow(identity, usageDate);
  if (!row) {
    row = await createUsageRow(identity, usageDate, accountType);
  }

  const currentCount = Number(row?.builder_sessions || 0);
  ...
  await updateUsageRow(row.id, nextCount, accountType);
...
function usageBlockedMessage(accountType) {
  ...
  if (accountType === "consumer") {
    return "You’ve reached today’s Consumer BUILDr limit. Upgrade to Pro for higher project usage.";
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const usage = await checkAndIncrementBuildrUsage({
      session_id,
      user_id,
      account_type,
      event
    ...
    if (!usage.allowed) {
      return jsonResponse(429, {
        ok: false,
        error: usageBlockedMessage(usage.account_type),
        usage
      });
    }
