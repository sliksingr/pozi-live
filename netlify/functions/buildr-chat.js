...
While gathering information, keep replies to one or two short lines. No Build Notes, no item list yet.

When you have enough to plan, deliver Build Notes and the POZi Item List in one message.
Group Build Notes under short plain labels with a blank line between groups. Use only what applies:
...
Aim for 12 to 18 bullets across the whole Build Notes section.
...
Every quantity in the item list must have its math in Build Notes.
Give real numbers, never placeholders: spacing in inches, lengths in feet, footing depth, rise and run.
State a waste allowance.

If a missing dimension changes the material count, ask for it. If the user will not answer,
state a common default, label it an assumption, and continue.

Measurements are common practice, not code approval. Keep the verify-local-code reminders.

FORMATTING

Output is plain text. Markdown is not rendered.
Never use asterisks for bold or italics. Never use # headings.
Labels are plain words on their own line. Use "- " for bullets.
...
// Must stay BELOW the Netlify function timeout so this AbortController fires first and
// returns a clean "took too long" message instead of an opaque platform kill.
// netlify.toml sets the function limit to 26s; this leaves 6s of headroom.
// 20s is needed because the final plan message (Build Notes + full item list) can run to
// the 900-token ceiling, which takes 8-15s on Haiku. Every earlier turn is far quicker.
...
async function checkForemanMessageLimit({ sessionId, tier, userEmail }) {
  ...
  // Per-session limit, reset daily. Two filters matter here:
  // - source_page: Foreman Vision logs to the SAME session_id under "pozi.vision" and has
  //   its own caps, so without this a photo analysis would burn a chat message.
  // - created_at: sessions persist across days so a contractor can return to a job and keep
  //   its full history. Without this the cap would be lifetime-per-session, and resuming
  //   tomorrow would cost a daily session slot while leaving zero messages to send.
