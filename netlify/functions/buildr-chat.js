...
/*
  ...
  Simple editable daily limits.
  This version does NOT use usage_tracking.
  It counts rows in the already-working buildr_chats table so we do not introduce a new permission path.
*/
...
async function countBuildrChatsToday({ session_id, user_id, event }) {
  ...
  const session = cleanIdentity(session_id);
  const ip = cleanIdentity(getClientIp(event));

  let identityFilter = "";

  if (user && user !== "guest" && user !== "anonymous") {
    identityFilter = `user_id=eq.${encodeURIComponent(user)}`;
  } else if (session && session !== "guest" && session !== "anonymous") {
    identityFilter = `session_id=eq.${encodeURIComponent(session)}`;
  } else if (ip) {
    identityFilter = `session_id=eq.guest_ip_${encodeURIComponent(ip)}`;
  } else {
    return 0;
  }
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/buildr_chats?select=id&${identityFilter}&created_at=gte.${since}`,
    {
      method: "GET",
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorText || "Unable to count BUILDr usage.");
  }
...
async function checkBuildrDailyLimit({ session_id, user_id, account_type, event }) {
  ...
  const usedToday = await countBuildrChatsToday({ session_id, user_id, event });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const fallback_ip = cleanIdentity(getClientIp(event));
    const session_id = raw_session_id || (fallback_ip ? `guest_ip_${fallback_ip}` : null);
