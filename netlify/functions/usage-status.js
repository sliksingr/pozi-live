...
// READ-ONLY Foreman usage status for the POZi Go account card.
//
...
// Enforcement + usage mutation live in buildr-chat.js and buildr-vision.js.
//
// IMPORTANT: the limit constants below must stay identical to the ones in buildr-chat.js
// and buildr-vision.js. If they drift, the app reports one number and enforces another.
//
...
const BUILDR_DAILY_LIMITS = Object.freeze({ guest: 1, free: 3, consumer: 5, pro: 7 });
const BUILDR_MESSAGES_PER_SESSION = Object.freeze({ guest: 5, free: 5, consumer: 10, pro: 15 });
const BUILDR_VISION_DAILY_LIMITS = Object.freeze({ guest: 0, free: 1, consumer: 10, pro: 25 });
...
// READ-ONLY: count distinct Foreman sessions already logged today.
...
// READ-ONLY: count Foreman Vision uses already logged today (source_page "pozi.vision").
...
// Reports what this function actually sees when resolving a tier. Reachable only at
// /.netlify/functions/usage-status?debug=1 while signed in. Remove once tiers work.
async function tierDiagnostic(userId) {
  const table = String(process.env.BUILDR_PROFILE_TABLE || "").trim();
  const planColumn = String(process.env.BUILDR_PLAN_COLUMN || "").trim();
  const idColumn = String(process.env.BUILDR_PROFILE_ID_COLUMN || "id").trim();
  const out = {
    user_id: userId,
    env_table_seen: table || "(EMPTY — env var not visible to this function)",
    env_plan_column: planColumn || "(EMPTY — env var not visible to this function)",
    env_id_column: idColumn
  };
  if (!table || !planColumn) {
    out.verdict = "Env vars missing at runtime, so getVerifiedTier() returns 'free' immediately.";
    return out;
  }
  const supabaseUrl = process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !key) { out.verdict = "SUPABASE_URL or SERVICE_ROLE_KEY missing."; return out; }

  const url = `${supabaseUrl}/rest/v1/${encodeURIComponent(table)}` +
    `?${encodeURIComponent(idColumn)}=eq.${encodeURIComponent(userId)}` +
    `&select=${encodeURIComponent(planColumn)}`;
  out.query_url = url.replace(supabaseUrl, "<SUPABASE_URL>");
  try {
    const r = await fetch(url, { headers: { apikey: key, Authorization: `Bearer ${key}` } });
    out.http_status = r.status;
    const body = await r.text();
    out.raw_response = body.slice(0, 500);
    if (!r.ok) { out.verdict = "REST call failed — see raw_response."; return out; }
    let rows = []; try { rows = JSON.parse(body); } catch {}
    out.rows_returned = Array.isArray(rows) ? rows.length : 0;
    out.plan_value = Array.isArray(rows) && rows[0] ? rows[0][planColumn] : null;
    out.verdict = out.rows_returned === 0
      ? "Query succeeded but returned NO ROW for this user_id."
      : `Found plan = ${JSON.stringify(out.plan_value)}.`;
  } catch (e) {
    out.verdict = "fetch threw: " + (e?.message || e);
  }
  return out;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // TEMPORARY: ?debug=1 returns tier-resolution diagnostics instead of usage.
    if (event.queryStringParameters?.debug === "1") {
      if (!userId) return jsonResponse(200, { ok: true, note: "Not signed in — send this request from a signed-in session." });
      return jsonResponse(200, { ok: true, diagnostic: await tierDiagnostic(userId) });
    }
