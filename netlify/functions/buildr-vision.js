...
//
// LIMITS: the constants below must stay identical to usage-status.js. See LIMITS.md.
//
...
// Must match buildr-chat-stream.mjs and usage-status.js. See LIMITS.md.
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        // The system prompt is ~1,900 identical tokens on every single analysis, so it is
        // marked for caching. Cache reads bill at 10% of base input. Nothing about the
        // prompt text changes — only how it is sent.
        system: [
          { type: "text", text: FOREMAN_VISION_SYSTEM_PROMPT, cache_control: { type: "ephemeral" } }
        ],
        messages: [{
          role: "user",
          content: [
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (imageData.length > MAX_IMAGE_BASE64_LENGTH) {
      return jsonResponse(413, { ok: false, error: "Image is too large for Foreman Vision. Please compress or resize the photo before sending." });
    }
