...
// The limit constants below must stay identical to usage-status.js. See LIMITS.md.
...
// ── LIMITS — must stay identical to usage-status.js. See LIMITS.md. ───────────
  ...
  try {
    ...
    const upstream = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        // PROMPT CACHING. Both prompts are long and byte-identical on every call, so they
        // are marked cacheable: cache reads bill at 10% of base input. The prompt TEXT is
        // unchanged — this only changes how it is sent, so the byte-identical rule across
        // files still holds. Chat is the big win, since the system prompt is a large share
        // of a short turn's input.
        system: [
          {
            type: "text",
            text: isPlan ? FOREMAN_PLAN_PROMPT : FOREMAN_SYSTEM_PROMPT,
