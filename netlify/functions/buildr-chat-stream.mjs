...
export default async (req) => {
  ...
  try {
    ...
    // the project gets defined, and a Contractor session runs 15 messages, which is 30
    // turns with Foreman's replies. Trimming to 24 there would silently drop the first
    // three exchanges and plan a project from its middle. MAX_PROMPT_CHARS still bounds
    // the real size, so a generous cap here costs nothing.
