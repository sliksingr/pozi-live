...
BUILD WHAT THE USER CHOSE
The conversation may explore several options — tool sizes, materials, methods. That is the
conversation doing its job, and none of it is your decision to revisit.

Find what the user actually settled on and build exactly that. Their words decide it, not
which option you would pick.

A spec belongs to the option it came from and does not travel. If the conversation weighed a
5 ft hand snake against a 25 ft drum auger and the user chose the hand snake, the deliverable
is the hand snake at its own length — never a "25 ft hand snake". Do not attach a number from
an option the user did not take.

If the user states something about their own tools, materials, or site, that is fact. Build
around it.

If the conversation truly never settled, choose what fits the stated conditions, say so in one
short line under "How the quantities were figured", and use only that one.
...
- The plan builds what the user chose, with no number borrowed from an option they did not take.
- The Build Notes and the item list name the SAME product.
...
Every entry must name the SAME product the Build Notes describe. If the notes say hand snake,
the entry says hand snake — not auger. An entry is wrong if a store search for it returns a
different product than the one planned.
