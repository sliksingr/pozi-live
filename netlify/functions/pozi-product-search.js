...
//
// SPLIT START/POLL (Netlify kills a synchronous function at 10s; DataForSEO can
// take longer than that to finish a task, so no single request may wait on it):
//   mode:"start" - POST creates the DataForSEO Shopping task and returns
...
//                  ~1s. The frontend calls this every ~2.5s for up to ~40s.
//
// Same filename and endpoint (/.netlify/functions/pozi-product-search) as
// before the split — the two call shapes are distinguished purely by the
...
async function handleStart(payload, login, password) {
  ...
  try {
    ...
    console.error("POZi product search (start) error:", error);
    return jsonResponse(timedOut ? 504 : 500, {
      ok: false,
      error: timedOut
        ? "POZi product task creation timed out."
...
async function handlePoll(payload, login, password) {
  ...
  if (!taskId) {
    return jsonResponse(400, { ok: false, error: "Missing task_id for product poll." });
  }
  ...
  try {
    ...
    if (!get.response.ok) {
      return jsonResponse(200, {
        ...
        pending: true,
        task_id: taskId,
        message: getDataForSeoError(get.data, "DataForSEO product result request failed.")
      });
    }
    ...
    // Genuine terminal error — surface it (frontend treats a caught error same
    // as an empty result and keeps the organic links visible).
    if (topStatus >= 40000 || taskStatus >= 40000) {
      return jsonResponse(502, {
        ok: false,
        error: taskMessage || "DataForSEO product task returned an error.",
    ...
    // the frontend's own ~40s budget is the real backstop.
    return jsonResponse(200, {
      ok: true,
      pending: true,
      task_id: taskId,
    ...
    const timedOut = error?.name === "AbortError";
    console.error("POZi product search (poll) error:", error);
    return jsonResponse(timedOut ? 504 : 500, {
      ok: false,
      error: timedOut
        ? "POZi product poll timed out."
        : error?.message || "Unknown POZi product search server error."
    });
  }
}
