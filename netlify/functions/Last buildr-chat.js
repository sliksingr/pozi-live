...
const BUILDR_MESSAGES_PER_SESSION = {
  guest: 5,
  free: 5,
  consumer: 10,
  pro: 15
...
function unlimitedUsagePayload(session_id) {
  return {
    allowed: true,
    tier: "test_unlimited",
    limit: 999999,
...
async function checkBuildrDailyLimit({
  session_id,
  user_id,
  user_email,
  account_email,
  ...
  if (isUnlimitedTestUser(user_id, user_email, account_email)) {
    return unlimitedUsagePayload(session_id);
  }
...
async function checkBuildrMessageLimit({
  session_id,
  tier,
  user_id,
  user_email,
  ...
  if (tier === "test_unlimited" || isUnlimitedTestUser(user_id, user_email, account_email)) {
    return unlimitedMessagePayload();
  }
  ...
  const limit =
    BUILDR_MESSAGES_PER_SESSION[tier] ??
    BUILDR_MESSAGES_PER_SESSION.guest;
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || "Unable to count BUILDr session messages.");
  }
...
function buildrMessageLimitMessage(tier) {
  if (tier === "guest") {
    return "You’ve reached the 5-message limit for this guest BUILDr session. Create a free account to keep planning.";
  }

  if (tier === "free") {
    return "You’ve reached the 5-message limit for this free BUILDr session. Start a new BUILDr session if you have sessions remaining today.";
  }

  if (tier === "consumer") {
    return "You’ve reached the 10-message limit for this Consumer BUILDr session. Start a new BUILDr session if you have sessions remaining today.";
  }

  if (tier === "pro") {
    return "You’ve reached the 15-message limit for this Pro BUILDr session. Start a new BUILDr session if you have sessions remaining today.";
  }
