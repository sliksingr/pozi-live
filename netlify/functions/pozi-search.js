...
//
// METHOD CHOICE — this stays on DataForSEO's LIVE endpoint, deliberately.
// Live returns synchronously in ~6s on average. DataForSEO's task-based
// Standard queue is NOT a viable alternative here: normal priority runs in
// minutes and even high priority averages ~1 minute, which is far beyond
// anything a user waiting on quick links will tolerate. Live is the correct
// method for an interactive, user-facing search.
//
// The occasional "search never loaded" case is the Live tail latency running
// past Netlify's 10s function limit and getting hard-killed mid-fetch. Fix is
// NOT a task queue — it's the timeout hardening below:
//   1. An AbortController caps the DataForSEO fetch at LIVE_TIMEOUT_MS (9s),
//      safely under Netlify's 10s, so we always return a clean JSON error
//      instead of dying mid-request with no response at all.
//   2. The frontend retries once on failure (see fetchOrganicRows in index).
// Product cards are a different story — that Shopping task genuinely exceeds
// 10s, which is why pozi-product-search.js uses the start/poll split.

const DATAFORSEO_ENDPOINT =
  "https://api.dataforseo.com/v3/serp/google/organic/live/regular";

// Cap the upstream fetch just under Netlify's 10s function limit so a slow
// DataForSEO response surfaces as a real error the app can retry, rather than
// the whole function being killed with nothing sent back.
...
exports.handler = async (event) => {
  ...
  // Real geo-targeting when the app resolved coordinates ("lat,lng,radius_km").
  // DataForSEO rejects location_name and location_coordinate together, so it's
  // one or the other — coordinates win, national name is the fallback.
  const locationCoordinate = String(payload.location_coordinate || "").trim();
  const validCoordinate = /^-?\d+(\.\d+)?,-?\d+(\.\d+)?(,\d+)?$/.test(locationCoordinate);
  ...
  if (validCoordinate) task.location_coordinate = locationCoordinate;
  else task.location_name = location;

  const dataForSeoBody = [task];

  const auth = Buffer.from(`${login}:${password}`).toString("base64");

  // Hard-cap the upstream call so we never get killed mid-fetch by Netlify.
  ...
  try {
    ...
    // AbortError = DataForSEO ran past our 9s cap. Return a clean, retryable
    // error instead of letting Netlify kill the function with no response.
    ...
    return jsonResponse(timedOut ? 504 : 500, {
      ...
      retryable: timedOut,
      error: timedOut
        ? "POZi search timed out upstream. Try again."
